...
const colorSchemes = {
    ...
    createdCollection: {
        ...
        tab: "bg-gray-200 group-hover:bg-gray-300",
    },
};
...
const CollectionPreview = ({
    ...
    if (isCreateNew){
        return <button onClick={onCreateNew} className='relative group h-[200px] cursor-pointer'>
            <FolderTab colorClass={colorSchemes["createdCollection"]?.bg} />
            <div className={`relative h-full rounded-lg p-6 shadow-md hover:shadow-lg transition-all flex flex-col 
                items-center justify-center gap-4 ${colorSchemes["createdCollection"]?.tab}`}>
  ...
  return (
    <Link href={`/collection/${isUnorganized ? "unorganized" : id}`} className='group relative'>
        <FolderTab colorClass={colorSchemes[isUnorganized ? "unorganized" : "collection"]?.tab} />
            <div className={`relative rounded-lg p-6 shadow-md hover:shadow-lg transition-all
                ${colorSchemes[isUnorganized ? "unorganized" : "collection"]?.bg}`}>
                <div className='flex items-center gap-3 mb-4'><span className='text-2xl'>
  {isUnorganized ? <FolderOpen className="w-5 h-5" /> : <Folder className="w-5 h-5" />}
</span>
                ...
                <div className='space-y-2'>
                    <div className='flex justify-between text-sm text-gray-600'>
                        ...
                        {entries.length>0 &&(
                            <span>
                               {entries[0]?.created_at && formatDistanceToNow(new Date(entries[0].created_at), { addSuffix: true })}
                            </span>
                        )}
                    </div>
                    <div>
